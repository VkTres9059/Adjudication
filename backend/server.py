...
# ==================== ELIGIBILITY MANAGEMENT ENDPOINTS ====================

@api_router.get("/members/eligibility/reconciliation")
async def eligibility_reconciliation(user: dict = Depends(get_current_user)):
    """Compare MGU Census vs latest TPA 834 feed snapshot. Flag ghost and unmatched members."""
...
@api_router.post("/members/eligibility/upload-tpa-feed")
...
@api_router.get("/members/eligibility/retro-terms")
async def retro_term_monitor(user: dict = Depends(get_current_user)):
    """Scan for retro-terminated members and identify claims paid after term date for clawback."""
...
@api_router.post("/members/{member_id}/request-refund")
async def request_provider_refund(member_id: str, user: dict = Depends(require_roles([UserRole.ADMIN]))):
    """Request provider refund for claims paid after retro-term date. Logs to clawback ledger."""
...
@api_router.post("/claims/process-pending-eligibility")
async def process_pending_eligibility(user: dict = Depends(require_roles([UserRole.ADMIN]))):
    """Process pending eligibility queue: auto-release if member now exists, auto-deny if 72hrs expired."""
    now = datetime.now(timezone.utc).isoformat()
...
@api_router.get("/members/eligibility/age-out-alerts")
async def age_out_alerts(user: dict = Depends(get_current_user)):
    """Find dependents turning 26 within the next 30 days for manual review or auto-termination."""
...
@api_router.get("/members/{member_id}/audit-trail")
...
async def create_claim(claim_data: ClaimCreate, user: dict = Depends(require_roles([UserRole.ADMIN, UserRole.ADJUDICATOR]))):
    ...
    # ===== PENDING ELIGIBILITY: Member not in census =====
    if not member:
        ...
        claim_doc = {
            ...
            **claim_data.model_dump(),
            ...
            "duplicate_info": None,
            "adjudication_notes": [
                f"PENDING ELIGIBILITY: Member {claim_data.member_id} not found in census. Claim held for 72 hours pending 834 feed update. Deadline: {eligibility_deadline}"
            ],
            "created_at": now,
            "created_by": user["id"],
            "adjudicated_at": None,
        ...
        await db.audit_logs.insert_one({
            ...
            "user_id": user["id"],
            "details": {"claim_id": claim_id, "member_id": claim_data.member_id, "deadline": eligibility_deadline},
            "timestamp": now
        })
